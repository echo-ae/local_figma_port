...
function Check-JsonServer {
    ...
    if ($server.env.SQLITE_PATH -ne (To-PosixPath $RepoSqlite) -or $server.env.DATA_DIR -ne (To-PosixPath $RepoData)) {
        Fail $Label
        return
    }
...
if ($ClaudeCode) {
    ...
    Check-JsonServer -Path (Join-Path $ProjectRoot ".mcp.json") -RepoMcpEntry $RepoMcpEntry -RepoSqlite $RepoSqlite -RepoData $RepoData -Label "Claude project MCP config points at repo build"
...
if ($Cursor) {
    Check-JsonServer -Path (Join-Path $ProjectRoot ".cursor/mcp.json") -RepoMcpEntry $RepoMcpEntry -RepoSqlite $RepoSqlite -RepoData $RepoData -Label "Cursor project MCP config points at repo build"
